...
- **Safe arithmetic on Android:** `FO_SAFE_ARITH_RELAXED` enables range-checked float/integer conversions and a permissive fallback path instead of build-stopping assertions in `clamp_to`; numeric casts now gate enum-underlying usage behind an explicit `numeric_like` concept so arithmetic types no longer instantiate `std::underlying_type` (`Source/Essentials/SafeArithmetics.h`).
...
- **Missing headers resolved:** Added `Source/Generated/Version-Include.h` stub (now also providing `FO_COMPATIBILITY_VERSION`) and exposed `Source/Frontend`/`Source/Frontend/Applications`/`Source/Generated` include directories so `Application.h`, `Rendering.h`, and `Version-Include.h` resolve during native builds.
