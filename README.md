...
- **Safe arithmetic on Android:** `FO_SAFE_ARITH_RELAXED` enables range-checked float/integer conversions and a permissive fallback path instead of build-stopping `static_assert(false)` branches in `clamp_to` (`Source/Essentials/SafeArithmetics.h`).
- **Geometry mode:** Android CMake defines `FO_GEOMETRY=2` (square) to satisfy the engine's required geometry switch (`app/src/main/cpp/CMakeLists.txt`).
...
- **Assimp optionality:** when `FO_HAVE_ASSIMP=0`, lightweight math stubs (`Source/Common/AssimpStubs.h`) provide the required matrix/vector/quaternion types so the client sources still compile. Android builds default to `FO_HAVE_ASSIMP=0` even without a CMake define to prevent missing-header errors.
